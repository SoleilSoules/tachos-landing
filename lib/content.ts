...
export const founder = deepNbsp({
  ...
  facts: ['Тех лид в части проектов — сам пишу код', 'На связи напрямую, без аккаунт-менеджеров'],
...
export const cases: CaseItem[] = deepNbsp<CaseItem[]>([
  ...
  },
  {
    id: 'maginary',
    client: 'Maginary',
    category: 'интерактивная книга',
    tabs: ['Геймдев'],
    // «Game of the Day» is already a tag pill on this card — the tail carries the
    // second figure instead of repeating it.
    desc: {
      lead: 'Интерактивная книга для iPhone, где текст оживает и реагирует на читателя —',
      highlight: '3 млн+ установок',
      tail: 'и 40 000+ отзывов',
    },
...
export const footer = deepNbsp({
  ...
  contacts: {
    email: { label: 'E-mail', value: 'vadim@tachos.ru' },
    phone: { label: 'Позвонить', value: '+7 988 002 3545' },
    city: 'город-герой Волгоград',
  },
  // Company details for the legal footnote under the contacts row. One array per
  // rendered line: registry numbers, then dates / activity, then the address.
  legal: {
    name: 'ООО «Тачос»',
    lines: [
      ['ОГРН 1133443024032', 'ИНН 3460009833', 'КПП 344401001'],
      ['Дата регистрации 03.09.2013', 'ОКВЭД 62.01'],
...
// Draft written from what the site actually does: two forms, no analytics, no
// cookies beyond the letter draft in localStorage. Needs a lawyer's pass before
// it can be relied on.
export const privacy = deepNbsp({
  title: 'Политика обработки персональных данных',
  updated: 'Редакция от 22 августа 2026 года',
  lead: 'Документ объясняет, какие данные о вас собирает сайт tachos.ru, зачем они нужны и что вы можете с ними сделать.',
  sections: [
    {
      title: '1. Кто обрабатывает данные',
      paragraphs: [
        'Оператор персональных данных — ООО «Тачос», ОГРН 1133443024032, ИНН 3460009833, КПП 344401001, адрес: 400131, г. Волгоград, наб. 62-й Армии, д. 6.',
        'Связаться по любому вопросу об обработке данных можно по адресу vadim@tachos.ru или по телефону +7 988 002 3545.',
      ],
    },
    {
      title: '2. Какие данные мы собираем',
      paragraphs: [
        'Через форму «Связаться» — контакт, который вы сами указываете для ответа (телефон, адрес электронной почты или имя пользователя в Telegram), и текст письма: тип задачи, её текущее состояние, сроки, ориентир по бюджету и описание своими словами, если вы его добавили.',
        'Через форму «Нашли ошибку?» — описание проблемы, скриншоты, если вы их приложили, адрес страницы, с которой отправлено сообщение, тип браузера и размер окна, а также адрес электронной почты, если вы захотели получить ответ.',
        'Мы не собираем данные скрытно: всё, что мы получаем, вы вводите сами в одну из этих двух форм.',
      ],
    },
    {
      title: '3. Зачем мы их обрабатываем',
      paragraphs: [
        'Единственная цель — ответить на ваше обращение и обсудить задачу, с которой вы пришли. Для сообщений об ошибках — воспроизвести проблему и починить сайт.',
        'Мы не используем ваши данные для рекламных рассылок, не продаём и не передаём их третьим лицам для их собственных целей.',
      ],
    },
    {
      title: '4. На каком основании',
      paragraphs: [
        'Основание обработки — ваше согласие, которое вы даёте, отмечая соответствующий пункт при отправке формы (пункт 1 части 1 статьи 6 Федерального закона № 152-ФЗ «О персональных данных»).',
        'Согласие можно отозвать в любой момент, написав на vadim@tachos.ru. После отзыва мы прекращаем обработку и удаляем данные, если закон не требует хранить их дольше.',
      ],
    },
    {
      title: '5. Сколько мы их храним',
      paragraphs: [
        'Обращение и контакт хранятся, пока идёт переписка по вашей задаче, и не дольше трёх лет с момента последнего контакта. После этого данные удаляются.',
        'Сообщения об ошибках хранятся в нашем трекере задач до тех пор, пока проблема не будет закрыта.',
      ],
    },
    {
      title: '6. Кому мы их передаём',
      paragraphs: [
        'Данные обрабатывают сотрудники студии, которым они нужны для ответа на ваше обращение.',
        'Кроме того, мы используем сторонние сервисы, через которые проходит отправка формы и хранение обращений. Такие сервисы обрабатывают данные по нашему поручению и не вправе использовать их для своих целей.',
      ],
    },
    {
      title: '7. Файлы cookie и аналитика',
      paragraphs: [
        'Сайт не использует счётчики посещаемости, рекламные пиксели и профилирующие файлы cookie.',
        'Черновик незаконченного письма сохраняется в памяти вашего браузера, чтобы форма не потеряла ответы, если вы её закрыли. Контакт и отметку о согласии мы туда не записываем, и этот черновик никуда не отправляется — он остаётся на вашем устройстве, пока вы не отправите письмо или не очистите данные сайта.',
      ],
    },
    {
      title: '8. Как мы их защищаем',
      paragraphs: [
        'Сайт работает по защищённому протоколу HTTPS. Доступ к обращениям есть только у сотрудников студии, которым он нужен по работе.',
      ],
    },
    {
      title: '9. Ваши права',
      paragraphs: [
        'Вы вправе узнать, какие ваши данные мы обрабатываем, потребовать их уточнения, блокировки или удаления, а также отозвать согласие.',
        'Для этого напишите на vadim@tachos.ru — мы ответим в срок, установленный законом, и в течение тридцати дней выполним требование.',
      ],
    },
    {
      title: '10. Изменения политики',
      paragraphs: [
        'Мы можем обновлять этот документ. Актуальная редакция всегда доступна по этому адресу, а дата редакции указана в начале страницы.',
      ],
    },
  ],
} as const);
