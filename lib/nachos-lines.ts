...
export function categorize(
  ...
  if (h.startsWith('Сюда придет')) return { category: 'contact', name: '' };
...
const CASE_FACTS: Record<string, string[]> = {
  Складно: [
    ...
    'Бронь, оплата, доступ — все в приложении. Это мы собрали.',
  ],
  Хайс: [
    'Банк для ИП. Собрали с чистого листа — счет, бухгалтерия, валюта.',
    'Анти-фрод зашили прямо в SDK. Тихо работает, надежно.',
    'Все, что ИП нужно от банка, — в одном экране. Наших рук дело.',
  ],
  Maginary: [
    'Книга, где ты сам решаешь сюжет. 750 тысяч скачали — не шутка.',
    'Целое приложение, где читатель — герой. Тапни, покажу.',
    'Анимированная книга-игра под iOS. Мы ее и собрали.',
  ],
};
...
const LINES: Record<Exclude<NachosCategory, 'case'>, string[]> = {
  ...
  role: [
    'Свой {name}, живьем. Никакого аутсорса.',
  ...
  founder: [
    'Это {name}, живьем. Отвечает сам.',
    '{name} — основатель. Без секретарей и прослоек.',
    'Напишешь сюда — попадешь прямо к {name}.',
  ],
  generic: [
