...
type Zone = 'hidden' | 'draft' | 'cases' | 'cta';

export function FloatingCompose() {
  const { open, isOpen, sent, hasDraft } = useCompose();
  const [zone, setZone] = useState<Zone>('hidden');

  useEffect(() => {
    // cta zone: focus line inside the (empty) #cta-zone marker
    const ctaInFocus = () => {
      const el = document.getElementById('cta-zone');
      if (!el) return false;
      const r = el.getBoundingClientRect();
      const focus = window.innerHeight * 0.62;
      return r.top <= focus && r.bottom >= focus;
    };
    // cases: only once the END of the case grid (past all 4 cards) is in the
    // lower part of the viewport — so it stays a small pill while reading cases
    const casesEndInView = () => {
      const el = document.getElementById('cases-end');
      if (!el) return false;
      const r = el.getBoundingClientRect();
      const vh = window.innerHeight;
      return r.top > vh * 0.2 && r.top < vh * 0.9;
    };
    const compute = () => {
      if (isOpen || window.scrollY < 620) {
        setZone('hidden');
        return;
      }
      if (ctaInFocus()) setZone('cta');
      else if (casesEndInView()) setZone('cases');
      else setZone('draft');
    };
    compute();
  ...
  const visible = zone !== 'hidden';
  const expanded = zone === 'cases' || zone === 'cta';
  const draftText = sent
    ? 'Письмо у Анны ✓'
    : hasDraft
      ? 'Черновик сохранён — продолжить'
      : 'Начните описывать задачу, мы поможем';
  const maxW = zone === 'cta' ? 1240 : zone === 'cases' ? 560 : 430;
  ...
  return (
    <div
      ...
      className={`group fixed bottom-[24px] left-1/2 z-[110] cursor-pointer overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
        ...
        expanded
          ? 'rounded-[28px] border border-white/10 bg-[#0c0c0c] text-white shadow-[0_24px_70px_rgba(0,0,0,0.5)]'
          : 'rounded-input bg-white text-black shadow-[0_16px_48px_rgba(0,0,0,0.3)]'
      }`}
    >
      {zone === 'cta' ? (
          <div className="flex items-center justify-between gap-[28px] p-[36px]">
            <div className="min-w-0">
              <p className="text-[26px] font-semibold leading-[1.15] tracking-[-0.01em]">{ctaBanner.title}</p>
              <p className="mt-[10px] text-[13px] uppercase tracking-[0.04em] text-white/40">{ctaBanner.note}</p>
            </div>
            <span className="flex shrink-0 items-center gap-[14px]">
              <span className="grid size-[44px] shrink-0 place-items-center rounded-full bg-white/10 text-[16px] font-semibold">
                А
              </span>
              <span className="flex items-center rounded-pill bg-accent px-[28px] py-[16px] text-[16px] font-medium text-white">
                {ctaBanner.cta}
              </span>
            </span>
          </div>
        ) : zone === 'cases' ? (
          <div className="flex items-center justify-between gap-[20px] py-[20px] pl-[28px] pr-[14px]">
            <span className="flex flex-col leading-tight">
              <span className="text-[20px] font-semibold tracking-[-0.01em]">Посмотреть все кейсы</span>
              <span className="mt-[3px] text-[13px] text-white/45">40+ цифровых продуктов в портфолио</span>
            </span>
            <span className="grid size-[48px] shrink-0 place-items-center rounded-full bg-accent text-white">
              <ArrowIcon />
            </span>
          </div>
        ) : (
          // small state — a miniature of the hero input: caret + placeholder + voice
          <div className="flex h-[58px] items-center gap-[10px] py-[8px] pl-[22px] pr-[8px]">
            <span
              className="h-[24px] w-[2.5px] shrink-0 bg-accent-hot [animation:caret-blink_1.1s_step-end_infinite]"
              aria-hidden
            />
            <span className="whitespace-nowrap pr-[4px] text-[15px] text-black/45">{draftText}</span>
            <span className="grid size-[42px] shrink-0 place-items-center rounded-full bg-surface2 text-black transition group-hover:bg-accent group-hover:text-white">
              <WaveIcon size={18} />
            </span>
          </div>
        )}
