...
// `letter: false` drops the whole «Расскажите нам о задаче» block — legal pages
// carry the footer for the contacts and the requisites, not to sell.
export function Footer({ letter = true }: { letter?: boolean } = {}) {
  ...
  return (
    ...
    >
      <div className="relative mx-auto max-w-page px-5 pb-[40px] pt-[64px] sm:px-10 lg:px-[96px] lg:pb-[54px] lg:pt-[100px]">
        ...
        <div
          className={`relative border-t border-white/10 pt-[28px] lg:pt-[40px] ${
            letter ? 'mt-[40px] lg:mt-[64px]' : ''
          }`}
        >
        ...
        {/* Company details. Sits close under the city row (Гоша), so it reads as
            its footnote; wraps on narrow screens instead of running off. */}
        <div className="mt-[14px] flex flex-col gap-[10px] sm:flex-row sm:items-start sm:justify-between sm:gap-[28px]">
          <div className="text-[13.5px] leading-[1.5] tracking-[0.02em] text-white/30">
            {footer.legal.lines.map((line, i) => (
              <div key={line.join()}>
                {i === 0 && <span className="text-white/40">{footer.legal.name} · </span>}
                {line.join(' · ')}
              </div>
            ))}
          </div>
          {/* the policy sits on the right, under the mail/phone pair above it */}
          <Link
            href="/privacy"
            className="shrink-0 text-[13.5px] leading-[1.5] tracking-[0.02em] text-white/45 underline decoration-white/20 underline-offset-[4px] transition hover:text-accent-bright sm:text-right"
          >
