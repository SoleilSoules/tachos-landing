...
float sparks(vec2 p, float t){
  ...
  for (int i = 0; i < 3; i++){
    ...
    if (h > 0.88){
      ...
      float d = length(f - c);
      s += smoothstep(0.14, 0.0, d) * (0.5 + 0.5*sin(t*6.0 + h*33.0));
    }
  }
  return s;
}
...
void main(){
  ...
  vec2 wp = fp + vec2(1.7, 0.9)*q;
  float neb = mix(fbm(wp*1.25), ridged(wp*1.6), 0.40);
  float pulse = 0.85 + 0.15*sin(uTime*0.3 + 0.6);
  float column = beam * (0.42 + 0.85*neb) * pulse;
  ...
  float glow = core*1.7 + column*1.15 + halo*0.26 + band*0.55 + sp*0.7;
