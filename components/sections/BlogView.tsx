  ...
  return (
    ...
    >
      ...
      <div className="space-y-3 border-l border-white/10 pl-4">
        {rows.map((r) => (
          <div key={r.k}>
            <p className="font-mono text-[11px] uppercase tracking-[0.04em] text-inverted/30">
              {r.k}
            </p>
      ...
      <div className="mx-auto max-w-[1100px] px-5 sm:px-8 lg:px-10">
        ...
        <div className="lg:pr-[252px]">
          ...
          <section className="mb-12 pt-8">
            ...
            <div
              className="mt-8 flex items-center justify-center overflow-hidden rounded-[6px] bg-white/[0.03]"
              style={{ minHeight: 360 }}
            >
              <svg
                width="32"
            ...
            {post.body.map((b, i) => {
              if (b.type === 'h') {
                return (
                  <h2
                    key={i}
                    className="mb-4 mt-12 text-[clamp(20px,2.2vw,26px)] font-semibold leading-[1.2] tracking-[-0.02em] text-inverted"
                  >
                    {b.text}
                  </h2>
                );
              ...
              if (b.type === 'quote') {
                return (
                  <blockquote
                    key={i}
                    className="my-10 border-l-2 border-accent pl-6 text-[clamp(20px,2.2vw,26px)] font-medium leading-[1.4] tracking-[-0.01em] text-inverted/90"
                  >
                    {b.text}
                  </blockquote>
                );
              ...
              return (
                <p
                  key={i}
                  className="mb-6 text-[clamp(16px,1.2vw,19px)] leading-[1.7] text-inverted/70"
                >
                  {b.text}
                </p>
              );
          ...
          <div ref={ctaRef} id="contacts">
            ...
            >
              ...
              <span className="font-mono text-[18px] tracking-[-0.02em] text-accent">
                {ctaBanner.cta} →
              </span>
            </button>
          </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                {others.map((p) => (
                  <Link
                    key={p.id}
                    href={`/blog/${p.slug}`}
                    className="group block rounded-[6px] bg-white/[0.04] px-5 py-5 transition-colors hover:bg-white/[0.07]"
                  >
                    <p className="mb-2 font-mono text-[11px] uppercase tracking-[0.04em] text-accent">
                      {p.tag}
                    </p>
                    <p className="text-[18px] font-semibold leading-[1.2] tracking-[-0.01em] text-inverted">
                      {p.title}
                    </p>
                    <p className="mt-2 font-mono text-[11px] text-inverted/30">
                      {p.date} · {p.read}
                    </p>
                  </Link>
                ))}
